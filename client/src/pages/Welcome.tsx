...
export default function Welcome() {
  ...
  useEffect(() => {
    getSetting('shortcutPTT', 'AltRight').then((k) => {
      const key = k as string
      setPttKey(key)
      setPttLabel(PTT_KEY_MAP[key] || key)
    })
  ...
  useEffect(() => {
    ...
    const onUp = (e: KeyboardEvent) => {
      ...
      if (pttPressed) {
        setPttPressed(false)
        setPttConfirmed(true)
        void (async () => {
          await setSetting('shortcutPTT', lastCode)
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (label) {
        ...
        setPttLabel(label)
        void (async () => {
          await setSetting('shortcutPTT', code)
          bridge.notifyShortcutsChanged()
        })()
      }
      ...
      case 0:
        return (
          <div className="flex flex-col items-center text-center">
            ...
            <p className="mt-1.5 text-sm text-muted-foreground/70">按住说话，松开即输入文本到任何应用</p>
          </div>
        )
        ...
        return (
          <div>
            ...
            <div className="space-y-3">
              {[
                { icon: Mic, title: '按住说话，松开输入', desc: '全局热键唤起，在任何应用中直接语音输入，无需切换窗口', color: 'text-blue-500 bg-blue-500/10' },
      ...
      case 2:
        return (
          <div className="flex flex-col items-center text-center">
            ...
            <h2 className="text-xl font-bold">设置按住说话热键</h2>
          ...
          <div>
            ...
            <Card className="mb-4">
              <CardContent className="flex items-center justify-between p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium">按住说话快捷键</p>
            ...
            <Card>
              <CardContent className="p-4">
                ...
                {canTest && (
                  <p className="mb-2 text-xs text-muted-foreground">请先点击下方文本框聚焦光标，然后按住 {pttLabel} 说话</p>
                )}
                <textarea
                  value={testText}
                  onChange={(e) => setTestText(e.target.value)}
                  placeholder={canTest
                    ? `点击这里，然后按住 ${pttLabel} 说话...`
        ...
        return (
          <div className="flex flex-col items-center text-center">
            ...
            <p className="mt-3 text-sm text-muted-foreground">
              在任何应用中按住 <span className="rounded border border-border bg-muted/50 px-1.5 py-0.5 font-medium text-foreground">{pttLabel}</span> 开始说话
            </p>
            <p className="mt-1.5 text-xs text-muted-foreground/70">松开后文字自动输入到光标位置</p>
          </div>
        )
